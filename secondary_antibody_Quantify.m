...
param_names = {'L1', 'L2', 'L3', 'L4', 'L5', 'phi1', 'phi2', 'phi3', 'phi4', 'phi5', 'theta1', 'theta2', 'theta3', 'theta4', 'theta5'};
mi_matrix = load_mi_matrix(15, 1);  
error_mi = mi_matrix(1:15, end);

max_probs = compute_max_prob_values(param_lists, param_names);

candidate_pairs = select_top_pairs(mi_matrix, param_names, 8);
...
for p = 1:size(candidate_pairs, 1)
    ...
    [range1, is_angle1] = get_param_range(idx1, param_lists, param_names);
    [range2, is_angle2] = get_param_range(idx2, param_lists, param_names);
    ...
    for i = 1:length(sampled_range1)
        for j = 1:length(sampled_range2)
            ...
            current_params = create_fixed_params(idx1, val1, idx2, val2, max_probs, param_names);
            
            temp_errors = zeros(1, 500);
            for k = 1:500
                params_k = assemble_params(current_params);
                XYZ = kinkedchain(params_k) + select_random_epitope(nsim);
                XYZ = add_hinge_noise(XYZ, noise_sigmas);
                valid = ~isclashing(XYZ, mindist);
                if valid
                    label_pos = compute_label_pos(XYZ);
                    dist = compute_dist(label_pos, r0);
                    offset = target_dist - dist;
                    label_pos_adj = label_pos + offset;
                    dist_adj = compute_dist(label_pos_adj, r0);
